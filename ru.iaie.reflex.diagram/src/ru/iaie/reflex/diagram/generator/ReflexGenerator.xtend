...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	{
		var String tempString = "";
		tempString += generateOneProcessNode(count_id, "     ", "circle")
		...
		{ //получаем список всех состояний процесса, и проходим по нему
	    	tempString += generateOneProcessNode(count_id, state.name, "roundrectangle") 
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
     	...
     	for (process : resource.allContents.toIterable.filter(Process)) 
		{
			fsa.generateFile(process.name + "_statechart_diagram.gml", generateStatechartDiagram(resource, process));
			clear()
		}
