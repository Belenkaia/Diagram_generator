/**
 * generated by Xtext 2.20.0
 */
package ru.iaie.reflex.diagram.reflex;

import org.eclipse.emf.ecore.EObject;

/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>Set State Stat</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * </p>
 * <ul>
 *   <li>{@link ru.iaie.reflex.diagram.reflex.SetStateStat#getStateId <em>State Id</em>}</li>
 * </ul>
 *
 * @see ru.iaie.reflex.diagram.reflex.ReflexPackage#getSetStateStat()
 * @model
 * @generated
 */
public interface SetStateStat extends EObject
{
  /**
   * Returns the value of the '<em><b>State Id</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>State Id</em>' attribute.
   * @see #setStateId(String)
   * @see ru.iaie.reflex.diagram.reflex.ReflexPackage#getSetStateStat_StateId()
   * @model
   * @generated
   */
  String getStateId();

  /**
   * Sets the value of the '{@link ru.iaie.reflex.diagram.reflex.SetStateStat#getStateId <em>State Id</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>State Id</em>' attribute.
   * @see #getStateId()
   * @generated
   */
  void setStateId(String value);

} // SetStateStat
